...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function storeJobEmail($data)
    {
        try {
            ...
            // Log the error
            Log::error('Error in immediateJobEmail: ' . $e->getMessage());

            // Return error response
            return response()->json(['error' => 'Unable to send immediate job email. Please try again.'], 500);
        }        
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    {
        // Retrieve the job to update by ID.
        $job = Job::find($id);
        ...
        // If no current translator is found, try to get the last completed translator.
